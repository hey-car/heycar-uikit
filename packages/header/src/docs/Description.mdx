...
`label` will often be the value passed in the label prop when setting up the link/button, or a value from the locale object.
...
const track = props => {
  //myTrackFn(props.label, props.action);
};
