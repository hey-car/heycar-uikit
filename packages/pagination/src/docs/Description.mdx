...
Without the `component` prop of `PaginationItem`, its component will be either a `button` or `a`. If `onClick` parm of `Pagination` is defined, it will be rendered as a button.
